...
class PfamScan(object):
    ...
    def _process_args(self, kwargs):
        ...
        if 'fasta' in kwargs and 'sequence' in kwargs:
            ...
            if not os.path.isfile(kwargs['fasta']):
                sys.exit('FATAL: fasta file "%s" doesn\'t exist' %
                         kwargs['fasta'])
        ...
        if 'e_seq' in kwargs:
            sys.exit(
                'FATAL: -e_seq functionality has not been implemented in Python')
        if 'e_dom' in kwargs:
            sys.exit(
                'FATAL: -e_dom functionality has not been implemented in Python')
        if 'b_seq' in kwargs:
            sys.exit(
                'FATAL: -b_seq functionality has not been implemented in Python')
        if 'b_dom' in kwargs:
            sys.exit(
                'FATAL: -b_dom functionality has not been implemented in Python')
        ...
        for hmmlib in self._hmmlib:
            ...
            if not os.path.isfile(os.path.join(self._dir, hmmlib)) and \
                ...
                sys.exit('FATAL: can\'t find %s and/or %s binaries in "%s' %
                         (hmmlib, hmmlib, kwargs['dir']))
        ...
        if self._translate:
            sys.exit(
                'FATAL: -translate functionality has not been implemented in Python')
    ...
    def _read_pfam_data(self):
        """
        Reads the Pfam data file ("pfam.hmm.dat") and populates the C<accmap>,
        ...
        for hmmlib in self._hmmlib:
            ...
            try:
                ...
                sys.exit('FATAL: Couldn\'t open "%s" data file: %s' %
                         (scandat, e.message))
            ...
            for line in SCANDAT:
                ...
                res_read_pfam_1 = re_read_pfam_1.search(
                    line)  # ^\#=GF ID\s+(\S+)
                res_read_pfam_2 = re_read_pfam_2.search(
                    line)  # ^\#=GF\s+AC\s+(\S+)
                res_read_pfam_3 = re_read_pfam_3.search(
                    line)  # ^\#=GF\s+DE\s+(.+)
                res_read_pfam_4 = re_read_pfam_4.search(
                    line)  # ^\#=GF\s+GA\s+(\S+)\;\s+(\S+)\;
                res_read_pfam_5 = re_read_pfam_5.search(
                    line)  # ^\#=GF\s+TP\s+(\S+)
                res_read_pfam_6 = re_read_pfam_6.search(
                    line)  # ^\#=GF\s+ML\s+(\d+)
                res_read_pfam_7 = re_read_pfam_7.search(
                    line)  # ^\#=GF\s+NE\s+(\S+)
                res_read_pfam_8 = re_read_pfam_8.search(
                    line)  # ^\#=GF\s+CL\s+(\S+)
    ...
    def _read_fasta(self):
        ...
        try:
            ...
            sys.exit('FATAL: Couldn\'t open fasta file "%s" %s' %
                     (self._fasta, e.message))
        ...
        for row in self._sequence_rows:
            ...
            if res_re_2:
                ...
                if seq_id in seq_hash:
                    sys.exit(
                        'FATAL: Sequence identifiers must be unique. Your fasta file contains two sequences with the same id (%s)' % seq_id)
    ...
    def search(self):
        ...
        if not self._sequence:
            sys.exit(
                'FATAL: no sequence given; set the search parameters before calling "search"')
        ...
        for hmmlib in self._hmmlib:
            ...
            if proc_err:
                sys.exit(
                    'An error was encountered while running hmmsearch: %s' % proc_err)

            self._hmmresultIO = HMMResultsIO()
            self._all_results = self._hmmresultIO.parseMultiHMMER3(
                proc_out_ascii)
            self._all_results = self._convert_results_search_to_scan(
                self._all_results)
        ...
        if len(AllResults) > 0:
            sys.exit(
                'FATAL: multiple search functionality has not been implemented in Python')
    ...
    def write_results(self, out, e_seq, e_dom, b_seq, b_dom):
        ...
        try:
            with open(out, 'w') as fh:
                ...
                fh.write('# %-25s %s\n' % ('Searching against:',
                                           os.path.join(self._dir, 'pfam.hmm')))
                ...
                for result in self._all_results:
                    self._hmmresultIO.write_ascii_out(
                        result, fh, self, e_seq, e_dom, b_seq, b_dom)

        except IOError as e:
            sys.exit('FATAL: Can\'t write to your output file "%s": %s' %
                     (out, e.message))
